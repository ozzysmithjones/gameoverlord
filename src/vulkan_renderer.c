...
static uint8_t compiled_frag_shader_spv[632] = {
    ...
    0x6C, 0x65, 0x72, 0x00, 0x05, 0x00, 0x04, 0x00, 0x11, 0x00, 0x00, 0x00,
...
static uint8_t compiled_vert_shader_spv[1044] = {
    0x03, 0x02, 0x23, 0x07, 0x00, 0x00, 0x01, 0x00, 0x0B, 0x00, 0x0D, 0x00,
    ...
    0x69, 0x6F, 0x6E, 0x00, 0x05, 0x00, 0x04, 0x00, 0x1C, 0x00, 0x00, 0x00,
    0x66, 0x72, 0x61, 0x67, 0x5F, 0x75, 0x76, 0x00, 0x05, 0x00, 0x04, 0x00,
    0x1D, 0x00, 0x00, 0x00, 0x69, 0x6E, 0x5F, 0x75, 0x76, 0x00, 0x00, 0x00,
...
static result create_shaders(renderer_internals* renderer, VkShaderModule* out_vert_shader, VkShaderModule* out_frag_shader) {
    ASSERT(renderer != NULL, return RESULT_FAILURE, "Renderer pointer cannot be NULL");
    ASSERT(out_vert_shader != NULL, return RESULT_FAILURE, "Output vertex shader pointer cannot be NULL");
    ASSERT(out_frag_shader != NULL, return RESULT_FAILURE, "Output fragment shader pointer cannot be NULL");
    ...
    if (vkCreateShaderModule(renderer->device, &vert_create_info, NULL, out_vert_shader) != VK_SUCCESS) {
    ...
    if (vkCreateShaderModule(renderer->device, &frag_create_info, NULL, out_frag_shader) != VK_SUCCESS) {
        BUG("Failed to create fragment shader module.");
        vkDestroyShaderModule(renderer->device, *out_vert_shader, NULL);
        return RESULT_FAILURE;
    }
...
#error Unsupported platform for Vulkan, add the code to get required window extensions here.
    ...
    if (create_shaders(r, &r->vert_shader_module, &r->frag_shader_module) != RESULT_SUCCESS) {
        BUG("Failed to create shaders.");
        return RESULT_FAILURE;
    }
...
void destroy_renderer(renderer* renderer) {
    ...
    vkDestroyShaderModule(r->device, r->frag_shader_module, NULL);
    vkDestroyShaderModule(r->device, r->vert_shader_module, NULL);
